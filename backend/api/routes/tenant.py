...
from backend.api.deps import SettingsDep, UserDep, clear_repo_cache
...
async def provision_tenant(settings: SettingsDep, user: UserDep) -> dict[str, Any]:
    ...
    if use_memory:
        ...
        updated = store.set_spreadsheet_id(record.id, sheet_id)
        # Seed default categories into isolated memory repo
    ...
    try:
        ...
        logger.exception("tenant provision failed for %s", record.email)
        raise HTTPException(
            status_code=503,
...
async def bind_spreadsheet(
    ...
    """
    Platform admin or owner bind: attach an existing spreadsheet id to the user.

    Body: ``{\"spreadsheet_id\": \"...\"}``
    Only the user themselves (or platform admin binding self) for v1 —
    platform admin can bind any user via ``user_id`` later; here self-only.
    """
    if not settings.multi_tenant:
        raise HTTPException(status_code=404, detail="Multi-tenant mode is disabled.")
    if not user.user_id:
    ...
    sheet_id = str(body.get("spreadsheet_id") or "").strip()
    if not sheet_id:
    ...
    store = get_control_store(settings)
    # Prevent two users mapping to the same sheet
    for other in store.list_users():
        if other.spreadsheet_id == sheet_id and other.id != user.user_id:
            raise HTTPException(
                status_code=409,
                detail="That spreadsheet is already bound to another account.",
            )

    updated = store.set_spreadsheet_id(user.user_id, sheet_id)
    clear_repo_cache()
    return {
        "status": "bound",
        "spreadsheet_id": sheet_id,
