...
if (-not $sid) {
  throw "SPREADSHEET_ID missing from .env - finish /setup first."
}
...
Write-Host "  2. Open Raw Editor (or bulk / shared variables paste)"
Write-Host "  3. Paste the FULL contents of railway-env.txt (replace old vars)"
Write-Host "  4. Save"
Write-Host "  5. Settings -> Networking -> Generate domain"
Write-Host "  6. Edit CORS_ORIGINS to that exact https URL (no trailing slash)"
Write-Host "  7. Redeploy if it does not auto-redeploy"
Write-Host ""
Write-Host "Opening railway-env.txt ..."
